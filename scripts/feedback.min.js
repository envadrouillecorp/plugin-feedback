...
loader = function() {
    ...
    $(div).append('<div>'+jGalleryModel.translate('Sending feedback... This might take a few minutes.')+'</div>');
...
window.Feedback = function( options ) {
    ...
    // default properties
    options.label = options.label || jGalleryModel.translate("Send feedback");
    options.header = options.header || jGalleryModel.translate("Send feedback");
    options.url = options.url || "/";
    options.adapter = options.adapter || new window.Feedback.XHR( options.url );
    
    options.nextLabel = options.nextLabel || jGalleryModel.translate("Continue");
    options.reviewLabel = options.reviewLabel || jGalleryModel.translate("Continue");
    options.sendLabel = options.sendLabel || jGalleryModel.translate("Send");
    options.closeLabel = options.closeLabel || "Close";
    
    options.messageSuccess = options.messageSuccess || jGalleryModel.translate("Your feedback was sent succesfully. Thanks!");
    options.messageError = options.messageError || jGalleryModel.translate("There was an error sending your feedback to the server.");
    ...
    if (options.pages === undefined ) {
        options.pages = [
            ...
            new window.Feedback.Captcha( options ),
    ...
    returnMethods = {
        ...
        open: function() {
            ...
            onclick_href = [];
            onclick_document = {};
            $('*').listHandlers('click', function(elem, cb) {
                 onclick_elts.push(elem);
                 onclick_cb.push(cb.handler);
                 onclick_cbargs.push(cb.data);
                 onclick_href.push($(elem).attr('href'));
                 $(elem).unbind('click');
                 if($(elem).attr('href'))
            ...
            $('a[href]').each(function(i, elem) {
                 onclick_elts.push(elem);
                 onclick_cb.push(null);
                 onclick_cbargs.push(null);
                 onclick_href.push($(elem).attr('href'));
                 if($(elem).attr('href'))
                     $(elem).removeAttr("href");
            });
            var doc_events = $(document).data('events');
            if(doc_events) {
               onclick_document.click = doc_events.click;
               onclick_document.live = doc_events.live;
               doc_events.click = doc_events.live = null;
            }
        ...
        close: function() {
            ...
            for ( var i =0; i < onclick_elts.length; i++) {
               if(onclick_cbargs[i] && onclick_cb[i]) {
                  $(onclick_elts[i]).click(onclick_cbargs[i], onclick_cb[i]);
               } else if(onclick_cb[i]) {
                  $(onclick_elts[i]).click(onclick_cb[i]);
               }
               if(onclick_href[i])
                  $(onclick_elts[i]).attr('href', onclick_href[i]);
            }
            var doc_events = $(document).data('events');
            if(doc_events) {
               doc_events.click = onclick_document.click;
               doc_events.live = onclick_document.live;
            }
            onclick_document = {};
            onclick_elts = [];
            onclick_cb = [];
...
window.Feedback.Form = function( elements ) {
    ...
    this.elements = elements || [{
        type: "textarea",
        name: "Issue",
        label: jGalleryModel.translate('Feedback (include your email if you want a response)'),
        required: false
    }];
...
window.Feedback.Captcha = function( options ) {
   this.url = options.url;
   this.captcha = options.captcha;
   this.refresh = options.refresh;
   this.captcha_validated = false;
   this.captcha_tries = 0;
   this.dom = document.createElement("div");
};

window.Feedback.Captcha.prototype = new window.Feedback.Page();

window.Feedback.Captcha.prototype.render = function() {
   var self = this;

   emptyElements( self.dom );
   this.captcha_validated = false;

   function change_captcha() {
      $(self.dom).find('#captcha').attr('src', self.captcha+'?rnd='+Math.random());
   }
   $(this.dom).append('<div id="captcha_error"></div>');
   $(this.dom).append('<table><tr><td style="vertical-align:middle"><img id="refresh" src="'+this.refresh+'" width="20px"/></td><td><img id="captcha" src="" /></td><td style="vertical-align:middle"><input type="text" id="captcha_valid"></input></td></tr></table>');
   $(this.dom).find('#refresh').click(change_captcha);
   change_captcha();
   return this;
};

window.Feedback.Captcha.prototype.end = function() {
   var self = this;
   if(this.captcha_validated)
      return true;

   $('.feedback-footer').find('.feedback-btn').attr('disabled', true);
   $.ajax({
      type: "POST",
      url: self.url,
      data: { code: $(self.dom).find('#captcha_valid').val() },
      dataType:'json',
      success:function( json ) {
         $('.feedback-footer').find('.feedback-btn').attr('disabled', null);
         if(json.valid === 'true' || json.valid === true) {
            self.captcha_validated = true;
            $('.feedback-footer').find('.feedback-btn').click();
         } else {
            $(self.dom).find('#captcha_error').html(jGalleryModel.translate('Invalid captcha'));
            self.captcha_tries++;
            if(self.captcha_tries > 1) {
               $(self.dom).find('#captcha_error').append(' ('+self.captcha_tries+')');
            }
         }
      },
      error:function() {
         $('.feedback-footer').find('.feedback-btn').attr('disabled', null);
         $(self.dom).find('#captcha_error').html('Error checking captcha');
      }
   });
...
window.Feedback.Captcha.prototype.start = function() {
    $('.feedback-footer').find('#c2').remove();
};

window.Feedback.Captcha.prototype.review = function( dom ) {
...
window.Feedback.Screenshot.prototype.start = function( modal, modalHeader, modalFooter, nextButton ) {
    if ( this.h2cDone ) {
       ...
       self.sendH2 = true;
       $('.feedback-footer').find('#c2').remove();
       $('.feedback-footer').append('<button id="c2" class="feedback-btn">'+jGalleryModel.translate('Continue without sending screenshot')+'</button>');
       $('.feedback-footer').find('#c2').click(function(event) {
          event.stopPropagation();
          self.sendH2 = false;
          $('.feedback-footer').find('.feedback-btn').click();
       });
       var nextButton2 = $('.feedback-footer').find('#c2')[0];
        ...
        this.mouseMoveEvent = function( e ) {
            ...
            if (e.target.nodeName === "BODY" ||  e.target === highlightClose || e.target === modal || e.target === nextButton || e.target.parentNode === modal || e.target.parentNode === modalHeader || e.target === nextButton2) {
        ...
        this.highlightClose = element("div", "Ś");
        ...
        this.dom.appendChild( element("p", jGalleryModel.translate("Highlight or blackout important information")) );
